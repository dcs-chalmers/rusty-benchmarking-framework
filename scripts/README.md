# How to use scripts
There are several useful scripts in this folder. `plot.py` is to plot data from benchmarks.
The shell scripts are for running benchmarks.

## Plotting
You need `pandas` and `matplotlib`.
To use `plot.py`, simply run it as follows:
```bash
...
./scripts/mpsc.sh array_queue,basic_queue,wf_queue 2 256 2 my_output
```
