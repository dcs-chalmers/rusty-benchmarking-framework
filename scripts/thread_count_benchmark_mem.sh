...
if [ $# -eq 0 ]; then
    echo "Usage: $0 <package1,package2,...> <thread-count-start> <thread-count-end> <thread-count-step> [output-dir] [spread]"
...
for PACKAGE in "${PACKAGE_ARRAY[@]}"; do
    ...
    cargo build --release -p "$PACKAGE" --features benchmark_core/memory_tracking

    # Loop through thread counts and run cargo command
    for ((i = START_THREADS; i <= END_THREADS; i += THREAD_STEP)); do
        echo "Running $PACKAGE with thread count: $i"
        time ./target/release/${PACKAGE} -t 1 -i 10 --path $OUTPUT/$PACKAGE enq-deq --thread-count $i --spread $SPREAD
    done
done
