...
impl LPRQueue {
    ...
    pub fn push(&self, item: *mut std::ffi::c_void) -> bool {
        ...
        trace!("{}: running lprq_push", self.get_thread_id());
    ...
    pub fn pop(&self) -> Option<*mut std::ffi::c_void> {
        ...
        let tid = self.get_thread_id();
        trace!("{}: running lprq_pop", self.get_thread_id());
        let success = unsafe { lprq_pop(self.raw, &mut item, tid) == 1 };
        trace!("{}: running lprq_pop done", self.get_thread_id());
        if success {
            Some(item)
        } else {
            None
...
impl Drop for LPRQueue {
    fn drop(&mut self) {
        trace!("{}: running lprq_destroy", self.get_thread_id());
        unsafe { lprq_destroy(self.raw) };
        trace!("{}: running lprq_destroy done", self.get_thread_id());
...
impl ConcurrentQueue<Box<i32>> for LPRQueue {
    ...
    fn new(_capacity: usize) -> Self {
        ...
        trace!("{}: Creating new lprq", 0);
        let raw = unsafe { lprq_create(MAX_THREADS) };
        trace!("{}: Creating new lprq done", 0);
