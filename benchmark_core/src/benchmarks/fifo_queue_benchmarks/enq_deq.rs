    ...
    for<'a> &'a C: Send
{
    ...
    let enq_deq_args = match &fifo_queue_args.benchmark_runner {
        FifoQueueBenchmarks::EnqDeq(a) => a,
        _ => panic!(
            "benchmark_enq_deq called with another FIFO Queue \
    ...
    let thread_count = enq_deq_args.thread_count;
    ...
    let _ = std::thread::scope(|s| -> Result<(), std::io::Error>{
        ...
        let &spread = &enq_deq_args.spread;
    ...
    stats.insert("Spread", Some(enq_deq_args.spread));
...
mod tests {
    ...
    use crate::arguments::FifoQueueEnqDeqArgs;
    ...
    fn run_pingpong() {
        let fifo_queue_args = FifoQueueArgs {
            benchmark_runner: FifoQueueBenchmarks::EnqDeq(
                FifoQueueEnqDeqArgs { thread_count: 10 , spread: 0.5 },
            ),
            ..Default::default()
        };
        ...
        assert!(benchmark_enq_deq(queue, &bench_conf, &fifo_queue_args, &mut stats).is_ok());
    }
